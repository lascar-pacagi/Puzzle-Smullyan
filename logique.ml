...
let rec eval (fm : formula) (v : valuation) : bool =
  ...
  | True  -> true
  | False -> false
  | P atom   -> get_value atom v
  | Not p -> not @@ eval p v
  | And (p1, p2) -> eval p1 v && eval p2 v
  | Or (p1, p2)  -> eval p1 v || eval p2 v
  | Imp (p1, p2) -> not @@ eval p1 v || eval p2 v
  | Iff (p1, p2) -> eval p1 v = eval p2 v
...
let atoms (fm : formula) : string list =
  let rec atoms fm acc =
    ...
    | Or (p1, p2)
    | Imp (p1, p2)
    | Iff (p1, p2) -> atoms p1 acc |> atoms p2
    | True | False -> acc
  in
  atoms fm []
...
(* let all_valuations fm =
 *   let rec all_valuations = function
 *     | [] -> [[]]
 *     | p :: r ->
 *        let res = all_valuations r in
 *        List.map (fun v -> (p, false) :: v) res
 *        @ List.map (fun v -> (p, true) :: v) res
 *   in
 *   atoms fm
 *   |> all_valuations
 *
 * let tautology2 fm =
 *   all_valuations fm
 *   |> List.for_all (eval fm) *)

let tautology (fm : formula) : bool =
  let rec evaluate_all v = function
    | [] ->
       eval fm v
    | p :: r ->
       evaluate_all ((p, false) :: v) r
      && evaluate_all ((p, true) :: v) r
  in
  atoms fm
  |> evaluate_all []
...
let rec nnf : formula -> formula = function
